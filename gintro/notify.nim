...
proc newNotification*(summary: cstring; body: cstring = ""; icon: cstring = ""): Notification =
  let gobj = notify_notification_new(summary, safeStringToCString(body), safeStringToCString(icon))
...
proc newNotification*(tdesc: typedesc; summary: cstring; body: cstring = ""; icon: cstring = ""): tdesc =
...
proc initNotification*[T](result: var T; summary: cstring; body: cstring = ""; icon: cstring = "") {.deprecated.} =
  assert(result is Notification)
  let gobj = notify_notification_new(summary, safeStringToCString(body), safeStringToCString(icon))
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
...
proc update*(self: Notification; summary: cstring; body: cstring = "";
    icon: cstring = ""): bool =
  toBool(notify_notification_update(cast[ptr Notification00](self.impl), summary, safeStringToCString(body), safeStringToCString(icon)))
