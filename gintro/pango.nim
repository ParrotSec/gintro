...
proc finalizerfree*(self: FontDescription) =
  if not self.ignoreFinalizer:
    pango_font_description_free(self.impl)
...
proc languageFromString*(language: cstring = ""): Language =
  let impl0 = pango_language_from_string(safeStringToCString(language))
  if impl0.isNil:
    return nil
...
proc finalizerunref*(self: AttrList) =
  if not self.ignoreFinalizer:
    pango_attr_list_unref(self.impl)
...
proc finalizerunref*(self: FontMetrics) =
  if not self.ignoreFinalizer:
    pango_font_metrics_unref(self.impl)
...
proc finalizerfree*(self: GlyphItem) =
  if not self.ignoreFinalizer:
    pango_glyph_item_free(self.impl)
...
proc finalizerfree*(self: GlyphItemIter) =
  if not self.ignoreFinalizer:
    pango_glyph_item_iter_free(self.impl)
...
proc finalizerfree*(self: GlyphString) =
  if not self.ignoreFinalizer:
    pango_glyph_string_free(self.impl)
...
proc setText*(self: Layout; text: cstring; length: int) =
...
proc finalizerfree*(self: LayoutIter) =
  if not self.ignoreFinalizer:
    pango_layout_iter_free(self.impl)
...
proc finalizerunref*(self: LayoutLine) =
  if not self.ignoreFinalizer:
    pango_layout_line_unref(self.impl)
...
proc finalizerfree*(self: TabArray) =
  if not self.ignoreFinalizer:
    pango_tab_array_free(self.impl)
...
proc drawGlyphItem*(self: Renderer; text: cstring = ""; glyphItem: GlyphItem;
    x: int; y: int) =
  pango_renderer_draw_glyph_item(cast[ptr Renderer00](self.impl), safeStringToCString(text), cast[ptr GlyphItem00](glyphItem.impl), int32(x), int32(y))
...
proc finalizerfree*(self: ScriptIter) =
  if not self.ignoreFinalizer:
    pango_script_iter_free(self.impl)
...
proc shapeFull*(itemText: cstring; itemLength: int; paragraphText: cstring = "";
    paragraphLength: int; analysis: Analysis; glyphs: GlyphString) =
  pango_shape_full(itemText, int32(itemLength), safeStringToCString(paragraphText), int32(paragraphLength), cast[ptr Analysis00](analysis.impl), cast[ptr GlyphString00](glyphs.impl))
...
proc shapeWithFlags*(itemText: cstring; itemLength: int; paragraphText: cstring = "";
    paragraphLength: int; analysis: Analysis; glyphs: GlyphString; flags: ShapeFlags) =
  pango_shape_with_flags(itemText, int32(itemLength), safeStringToCString(paragraphText), int32(paragraphLength), cast[ptr Analysis00](analysis.impl), cast[ptr GlyphString00](glyphs.impl), flags)
